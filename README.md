...
Build the frontend (Caddy serves the static Vite output under `frontend/dist`). For a production-like bundle, pass the same **build-args** CI uses (they are mapped to **`VITE_*`** inside the Dockerfile):
